...
export default class UserServices implements IUserServices {
    ...
    async Signup(name: string, username: string, email: string, password: string):Promise<string> {
    ...
    async getByID(ID: string):Promise<User> {
        ...
        const responce = Axios.get(`https://localhost:5001/api/user/${ID}
    ...
    async Update(user: User, image: string | ArrayBuffer):Promise<User> {
        user.profileImage = image.toString();
        const AuthStr = TokenServices.getAuthString();
        const responce = Axios.post(`https://localhost:5001/api/user/UpdateUser`, user,
            { headers: { Authorization: AuthStr } }
        );
